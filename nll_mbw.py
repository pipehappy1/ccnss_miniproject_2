...
def weighted_nll(alphap,betap,w1p, sub_indx):
    ...
    for t in range(n_trials):
        ...
        ll += np.log(softmax_score(state, w1*q+w2*qb, beta,real_action))
...
for sub in range(n_subs):
    ...
    for i in range(n_try):
        ...
        value = weighted_nll(alpha, beta, weight, start_index[sub])
        if value < best_value:
            best_alpha = alpha
            best_beta = beta
            best_weight = weight
    ...
    print("[{}, {}, {}, {}],".format(best_value, best_alpha, best_beta, best_weight))
